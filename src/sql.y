...
column_def :
			...
			| column_def constraint_name_opt L_NOT _NULL_ 															{if($1->column_def.is_not_null){delete_table_element($1); deinit_dstring(&($2)); YYABORT;} $1->column_def.is_not_null = 1; $$->column_def.is_not_null_constraint_name = $2; $$ = $1;}
			| column_def constraint_name_opt PRIMARY_KEY 															{if($1->column_def.is_primary_key){delete_table_element($1); deinit_dstring(&($2)); YYABORT;} $1->column_def.is_primary_key = 1; $1->column_def.is_primary_key_constraint_name = $2; $$ = $1;}
			| column_def constraint_name_opt UNIQUE 																{if($1->column_def.is_unique){delete_table_element($1); deinit_dstring(&($2)); YYABORT;} $1->column_def.is_unique = 1; $$->column_def.is_unique_constraint_name = $2; $$ = $1;}
			| column_def constraint_name_opt REFERENCES IDENTIFIER OPEN_BRACKET IDENTIFIER CLOSE_BRACKET 			{if($1->column_def.is_foreign_key){delete_table_element($1); deinit_dstring(&($2)); deinit_dstring(&($4)); deinit_dstring(&($6)); YYABORT;} $1->column_def.is_foreign_key = 1; $$->column_def.foreign_table = $4; $$->column_def.foreign_column = $6; $$->column_def.is_foreign_key_constraint_name = $2; $$ = $1;}
			| column_def constraint_name_opt REFERENCES IDENTIFIER  												{if($1->column_def.is_foreign_key){delete_table_element($1); deinit_dstring(&($2)); deinit_dstring(&($4)); YYABORT;} $1->column_def.is_foreign_key = 1; $$->column_def.foreign_table = $4; $$->column_def.is_foreign_key_constraint_name = $2; $$ = $1;}
...
type :
		...
		| type_name with_or_without_timezone ARRAY dims_list												{$$ = new_sql_type($1); $$->with_time_zone = $2; $$->for_array = 1; $$->array_dims_size = $4[0]; memcpy($$->array_dims, &($4[1]), sizeof(int64_t) * $4[0]);}
